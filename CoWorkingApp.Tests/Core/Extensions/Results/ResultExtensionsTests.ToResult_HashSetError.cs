using CoWorkingApp.Core.Extensions;
using CoWorkingApp.Core.Shared;
using ERRORS = CoWorkingApp.Core.DomainErrors.Errors;

namespace CoWorkingApp.Tests.Core.Extensions.Results;

public partial class ResultExtensionsTests
{
    public class ToResultHashSetErrorTests
    {
        [Fact]
        public void ToResult_ShouldCreateFailureResult_FromHashSetOfErrors()
        {
            // Arrange
            HashSet<Error> errors = [ERRORS.NullValue, ERRORS.Unknown];

            // Act
            var result = errors.ToResult<string>();

            // Assert
            Assert.False(result.IsSuccess);
            Assert.True(errors.SetEquals(result.Errors));
        }
    }
}
